...
class LogParser:
    ...
    def WriteEventToFile(self):
        ...
        for i in range(len(eventList)):
            LogKeylist.append([i+1, eventList[i][0],eventList[i][1]])
            keyDict[LogKeylist[i][1]] = LogKeylist[i][0]

        LogKeyDf = pd.DataFrame(LogKeylist, columns=['LogKey', 'patternid', 'pattern'])
        LogKeyDf.to_csv(os.path.join(self.para.savePath, self.logname + '_LogKey.csv'), index=False)
        ...
        #workflow
        vec = []
        for i in self.df_log["EventId"] :
            vec.append(keyDict[i])
        vecfile = open(self.para.savePath+'vectorize', 'w')
        ten = 0
        for i in vec :
            vecfile.write(str(i))
            vecfile.write(' ')
